...
2. linux `cargo test` currently cannot be the full baseline due missing `gobject-2.0` on linux host; gui verification must stay mac-executed for now.
...
1. upgrade `dev/mac/e2e_interactions.bash` assertions:
...
1. standardize artifact layout for all gui checks:
...
1. add one entrypoint script `dev/mac/run_verification_suite.bash`:
...
1. add optional html report generator `dev/linux/render_gui_report.bash`:
...
1. document “single command from linux”:
...
1. add an optional `appium-mac2` lane in `dev/mac/e2e_appium/` using webdriver + appium mac2 driver for native interactions and built-in screenshot/recording commands, scoped to high-value flows only: launch, drag, resize, context menu pause/resume.
...
1. animation present: startup unpaused, motion variance above threshold, distinct phase extrema detected.
2. tempo correctness: default tempo estimates near 5.5s half-cycle, and menu `fast`/`slow` produce expected shorter/longer half-cycle.
3. pause/resume: pause yields near-flat luma series; resume restores oscillation.
4. interaction behavior: drag moves window beyond minimum delta, coarse wheel resize delta >= 2 px, and shift+wheel fine delta in 1-3 px.
5. persistence behavior: `settings.toml` reflects pause/speed/size changes and survives restart.
6. artifact integrity: every run emits mandatory files; suite fails if artifact set incomplete.
...
1. chosen defaults: strategy is hybrid phased, primary artifact is video + keyframes, and app instrumentation is disallowed (no new in-app debug/test channels).
...
4. tolerance defaults: half-cycle tolerance `±0.7s`, and pause variance threshold tuned empirically in first implementation pass and then frozen in docs.
