...
3. pii/sensitive exclusions (hard rules)
...
2. errors/crashes (better stack errors via sentry)
...
2. add persistent identity and queue storage (`telemetry.toml`, ndjson spool).
3. add better stack logs sink and retry/backoff batching.
4. add sentry sink and panic hook wiring.
5. wire app lifecycle hooks in `src/main.rs` for install/session/error/crash events.
6. add menu action events for `pause`, `resume`, `size_change`, `reset`, `quit`, `analytics_menu`.
7. implement split consent submenu + `What we collect…` modal in webview html/js + new ipc commands.
8. persist new consent fields in settings and apply runtime enable/disable.
9. add/update docs for env vars and privacy behavior.
10. add tests.
