...
- artifact-based mac gui verification is available through the `gui-smoke-macos` github actions workflow; it downloads the latest published dmg, launches `Downshift.app`, checks for a visible app window, triggers a warmup capture, and records cropped screenshot plus diff artifacts.
